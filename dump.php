...
else
{
		...
		WHILE (( $dump['table_offset'] < $num_tables ) && ( $dump['restzeilen'] > 0 ))
		{
			...
			if ($dump['zeilen_offset'] == 0)
			{
				...
				else
				{
					...
					$read_create_error=sprintf($lang['fatal_error_dump'],$table,$adbname) . ': ' . mysql_error($config['dbconnection']);
	...
	if ($config['multi_dump'] == 1)
	{
		...
		for ($i=0; $i < $count_dbs; $i++)
		{
			...
			else $mudbs.='<span class="success">' . $databases['multi'][$i] . '&nbsp;&nbsp;</span> ';
		}
	}
	...
	$aus[]=$lang['dump_filename'] . '<b>' . $dump['backupdatei'] . '</b><br>' . $lang['charset'] . ': <strong>' . $dump['dump_encoding'] . '</strong>' .
	...
	if (isset($dump['tables'][$dump['table_offset']]))
	{
		...
		$aus[]=$lang['saving_table'] . '<b>' . ( $dump['table_offset'] + 1 ) . '</b> ' . $lang['of'] . '<b> ' . sizeof($dump['tables']) . '</b><br>' . $lang['actual_table'] . ': <b>' . $table . '</b><br><br>' . $lang['progress_table'] . ':<br>';
		...
		$aus[]='</tr><tr>' . '<td colspan="3">' . $lang['entry'] . ' <b>' . number_format($eintrag,0,',','.') . '</b> ' . $lang['upto'] . ' <b>' . number_format(( $zeilen_gesamt ),0,',','.') . '</b> ' . $lang['of'] . ' <b>' . number_format($dump['zeilen_total'],0,',','.') . '</b></td></tr></table>';
	...
	else
	{
		...
		if ($config['multi_part'] == 1)
		{
			...
			$out.="\n" . '<div class="backupmsg">' . $lang['file'] . ' <a href="' . $config['paths']['backup'] . $dump['backupdatei'] . '" class="smallblack">' . $dump['backupdatei'] . ' (' . byte_output(filesize($config['paths']['backup'] . $dump['backupdatei'])) . ')' . '</a>' . $lang['dump_successful'] . '<br>';
		...
		$aus[]='<form action="" method="POST">' . $out . '<br>' . '<p class="small">' . zeit_format($xtime) . ', ' . $dump['aufruf'] . ' pages' . $aus2 . '</p>' . "\n";
