...
function WriteCronScript($restore_values=false)
{
    ...
    // only one db selected - get values from old array
    if ($config['cron_dbindex'] >= 0) {
        unset($cron_db_array, $cron_dbpraefix_array, $cron_command_before_dump, $cron_command_after_dump);
        $cronDbIndex = 0; //$config['cron_dbindex'];
        $cron_db_array[0] = $databases['Name'][$config['cron_dbindex']];
        $cron_dbpraefix_array[0]=$databases['praefix'][$config['cron_dbindex']];
        $cron_command_before_dump[0]=$databases['command_before_dump'][$config['cron_dbindex']];
        $cron_command_after_dump[0]=$databases['command_after_dump'][$config['cron_dbindex']];
    } else {
