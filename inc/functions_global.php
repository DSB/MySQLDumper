...
function WriteLog($aktion) {
	...
	if ($config ['logcompression'] == 1) {
		...
		if ($fp) {
			...
			echo '<p class="warnung">' . $lang ['logfilenotwritable'] . ' (' . $logfile . ')</p>';
	}
	else {
		$fp = @fopen ( $logfile, "ab" );
		if ($fp) {
				...
				foreach ( $db_files as $timestamp => $filenames ) {
					if ($i >= $config ['max_backup_files']) {
						...
						foreach ( $filenames as $f ) {
							if ($out == '') $out .= $lang ['fm_autodel1'] . '<br>';
							if (@unlink ( './' . $config ['paths'] ['backup'] . $f )) {
								$out .= '<span class="nomargin">' . sprintf ( $lang ['delete_file_success'], $f ) . '</span><br>';
							}
							else {
								$out .= $lang ['error'] . ': <span class="error nomargin">' . sprintf ( $lang ['delete_file_error'], $f ) . '</span><br>';
							}
						}
					}
					$i ++;
...
function DeleteFile($files, $function = 'max') {
	...
	$r = '<p class="error">' . $lang ['fm_autodel1'] . '<br>';
	$r .= $delfile [3] . "<br>";
	$part = $delfile [2];
	if ($part > 0) {
		for($i = $part; $i > 0; $i --) {
...
function NextPart($s, $first = 0, $keep_suffix=false) {
	...
	//if ($keep_suffix && substr($s,-3)=='.gz') $filename.='.gz';
	return $filename;
}
...
// Rekursives entfernen von Slashes in beliebig tief verschachtelten Arrays
