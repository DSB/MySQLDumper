 ...
 *
 * Changes in 1.0.1:
 * - New location case "mc" for center direction messages
 *
 * @todo
 */
Growler = (function(){
	...
	function createNotice(growler, msg, options){
		...
		new Effect.Opacity(notice, { to: 0.95, duration: opt.speedin });
		if (!opt.sticky){
			...
			if(IE==6){
				...
				switch(opt.location){
					...
					case "bc":
						...
						break;
					case "ml":
						this.growler.setStyle({top: "40%", left: "0", width: "50%"});
						break;
					case "mc":
						this.growler.setStyle({top: "40%", left: "25%", width: "50%"});
						break;
					case "mr":
						this.growler.setStyle({top: "40%", right: "0", width: "50%"});
						break;
					default:
