...
<br>
1. Ejecute MySQLDumper desde su navegador, y vaya a la p&aacute;gina de <br>
...
<ul>
	...
	<ul><li><b><?=$lang['Statusinformationen']?></b> le mostrar&aacute; las informaciones gen&eacute;ricas, pudiendo adem&aacute;s acceder a algunas de ellas en particular, para ampliarlas.</li>
	  <li><b><?=$lang['dbs']?></b> le llevar&aacute; a la lista de las mismas, pudiendo crear otras nuevas. Si hace click en alguna de ellas, se le llevar&aacute; a un men&uacute; avanzado d&oacute;nde se le mostrar&aacute;n las tablas que contiene y distintas opciones para con ellas. </li>
	  <li><b><?=$lang['mysqlvars']?></b> le mostrar&aacute; respectivamente los procesos, el estado y las opciones y definiciones del servidor de base de datos MySQL. </li>
	  <li><b><?=$lang['mysqlsys']?></b> le permitir&aacute; acceder a una pseudoconsola del servidor de bases de datos y realizar operaciones complejas con el mismo. Nota: para poder utilizar dichas opciones, deber&aacute; tener los privilegios adecuados en el servidor MySQL.</li>
	</ul>
	...
	<ul>
		<li><b><?=$lang['dbs']?></b> le permite cambiar los par&aacute;metros de conexi&oacute;n (haga click en mostrar / esconder) del usuario de base de datos.  Si hay m&aacute;s de una base de datos, puede elegir hacer un volcado m&uacute;ltiple e incluir m&aacute;s de una bases de datos en la copia de seguridad (la base de datos actual se muestra siempre en <b>negrita</b>). Adem&aacute;s, podr&aacute; seleccionar las tablas que ser&aacute;n incluidas en la copia de seguridad mediante un prefijo.</li>
		<ul>
		  <li><a name="conf1"></a><b><?=$lang['help_db']?></b> muestra un listado de todas las bases de datos accesibles. Si ha especificado en los par&aacute;metros de conexi&oacute;n, que solamente se muestre un tipo una base de datos, solamente aparecer&aacute; &eacute;sta. Si hay m&aacute;s de una base de datos accesible, puede elegir hacer un volcado m&uacute;ltiple e incluir m&aacute;s de una bases de datos en la copia de seguridad (la base de datos actual se muestra siempre en <b>negrita</b>). Adem&aacute;s, podr&aacute; seleccionar las tablas de cada base de datos que deben ser incluidas en la copia de seguridad mediante un prefijo, excluyendo las que no contengan el mismo.</li>
		...
		<li><b><?=$lang['general']?></b> sirve para elegir las caracter&iacute;sticas gen&eacute;ricas de las copias de seguridad (compresi&oacute;n, memoria, velocidad [ALERTA: la velocidad excesiva puede provocar que el servidor deje de responder (timeout)], archivos de registro o logs, si se deben optimizar las tablas antes de hacer la copia, etc...) y de la restauraci&oacute;n de datos (si se deben vaciar las tablas antes de hacerla, si se debe detener la importaci&oacute;n en caso de errores).</li>
		<ul>
		  <li><a name="conf3"></a><b><?=$lang['gzip']?></b> permite activar la compresi&oacute;n de los archivos. Se recomienda activarla, si el m&oacute;dulo GZIP est&aacute; disponible en su servidor, ya que el tama&ntilde;o de los archivos se reduce sensiblemente.</li>
       ...
       permite vaciar el contenido de la base de datos totalmente antes de realizar la recuperaci&oacute;n de datos de una copia de seguridad existente. Es recomendable en caso de recuperar una serie de tablas que se hayan corrompido. En caso de duda, se recomienda dejarlo desactivado, puesto que si realiza una recuperaci&oacute;n parcial de algunas tablas, se eliminar&iacute;an antes todas las tablas existentes, aunque no se encuentren presentes en la copia de seguridad a recuperar.</li>
		...
		<li><b><?=$lang['config_interface']?></b> permite elegir las caracter&iacute;sticas gr&aacute;ficas de la interfaz del programa. Puede elegir idioma, tema, definir algunos tama&ntilde;os de ventana, incluso decidir si desea que aparezca el nombre del servidor en que se encuentra en este momento, y en qu&eacute; lugar. La elecci&oacute;n del navegador que utiliza es importante, ya que si lo hace de forma incorrecta, el programa no funcionar&aacute; correctamente. </li>
		<ul>
			<li><a name="conf11"></a><b><?=$lang["help_lang"]?>:</b> aqu&iacute; puede seleccionar el idioma para el interfaz gr&aacute;fico.</li>
		</ul>
		<li><b><?=$lang['config_autodelete']?></b> define los par&aacute;metros que determinan si se van a eliminar archivos de copia de seguridad de forma autom&aacute;tica o no.</li>
		<ul>
			<li><a name="conf8"></a><b><?=$lang["help_ad1"]?>:</b> activa o desactiva la eliminaci&oacute;n augom&aacute;tica. Si est&aacute; activado, se eliminar&aacute;n los archivos necesarios (seg&uacute;n las reglas definidas a continuaci&oacute;n) antes de iniciar una nueva copia de seguridad. Es una opci&oacute;n &uacute;til para ahorrar espacio en el servidor, pero le recomendamos que no la active antes de haber podido probar el funcionamiento correcto del programa.</li>
			<li><a name="conf9"></a><b><?=$lang["help_ad2"]?>:</b> un valor mayor que cero elimina todos los archivos de copia de seguridad que tengan mayor antig&uuml;edad que la escrita.</li>
			<li><a name="conf10"></a><b><?=$lang["help_ad3"]?>:</b> un valor mayor que cero elimina todos los archivos de copia de seguridad en exceso del n&uacute;mero especificado, bien en total, bien para cada base de datos distinta.</li>
		</ul>
		<li><b><?=$lang['config_email']?></b> define los par&aacute;metros que determinan si se va a enviar un email tras haber completado una copia de seguridad, as&iacute; como si se deber&aacute; adjuntar dicha copia de seguridad y en qu&eacute; forma.</li>
		<ul>
			<li><a name="conf8"></a><b><?=$lang["help_mail1"]?>:</b> activa o desactiva el env&iacute;o de un email al haberse terminado la copia de seguridad, sea con o sin &eacute;xito.</li>
			<li><a name="conf9"></a><b><?=$lang["help_mail2"]?>:</b> es la direcci&oacute;n de email a d&oacute;nde se enviar&aacute; el mensaje.</li>
			<li><a name="conf10"></a><b><?=$lang["help_mail3"]?>:</b> es la direcci&oacute;n de email desde donde se enviar&aacute; el mensaje. Recuerde permitirle el paso a trav&eacute;s de su filtro anti-spam, si dispone de uno.</li>
		</ul>
		...
		<ul>
			<li><a name="conf13"></a><b><?=$lang["help_ftptransfer"]?>:</b> activa o desactiva el env&iacute;o de la copia de seguridad realizada con &eacute;xito, por FTP.</li>
			<li><a name="conf14"></a><b><?=$lang["help_ftpserver"]?>:</b> es la direcci&oacute;n del servidor de FTP destinatario del archivo (por ejemplo ftp.misbackups.com).</li>
			<li><a name="conf15"></a><b><?=$lang["help_ftpport"]?>:</b> es el puerto de conexi&oacute;n del servidor FTP (generalmente, el puerto 21).</li>
			<li><a name="conf16"></a><b><?=$lang["help_ftpuser"]?>:</b> es el nombre de usuario con el que debe realizarse la conexi&oacute;n.</li>
			<li><a name="conf17"></a><b><?=$lang["help_ftppass"]?>:</b> es el password a utilizar para establecer la conexi&oacute;n.</li>
		...
		<li><b><?=$lang['config_cronperl']?></b> permite definir las caracter&iacute;sticas de la copia de seguridad utilizando el script Perl. La mayor&iacute;a de las opciones son muy parecidas a las establecidas para el interfaz gr&aacute;fico, pero para el script Perl, que act&uacute;a de forma independiente.</li>
	</ul>
	<h6><?=$lang['dump']?></h6>
...
<p class="verysmall" style="color:green;">No somos responsables del contenido de las siguientes p&aacute;ginas. Si hace click en alguno de los enlaces detallados a continuaci&oacute;n, abandonar&aacute; nuestra aplicaci&oacute;n y se encontrar&aacute; en p&aacute;ginas de internet sobre las que no tenemos ning&uacute;n control.</p>
