...
if(isset($_POST["htaccess"]) || $action=="schutz") {
	...
	//Ausgabe
	echo headline('<img src="images/key.gif" alt="" width="29" height="33" hspace="16" border="0">'.$lang["htaccess1"]).$Overwrite;
	if($step<2){
...
if($action=="edithtaccess") {
	...
	echo headline('<img src="images/key.gif" alt="" width="29" height="33" hspace="16" border="0">'.$lang["htaccess16"]);
...
echo '<td><input class="Formbutton" type="Button" onclick="location.href=\'main.php?action=status\'" value="'.$lang['Statusinformationen'].'"></td>';
echo '<td><input class="Formbutton" type="Button" onclick="location.href=\'main.php?action=db\'" value="'.$lang['dbs'].'"></td>';
echo '<td><input class="Formbutton" type="Button" onclick="location.href=\'main.php?action=vars\'" value="'.$lang['mysqlvars'].'"></td>';
echo '<td><input class="Formbutton" type="Button" onclick="location.href=\'main.php?action=sys\'" value="'.$lang['mysqlsys'].'"></td>';
...
if($action=="status") {
	...
	$status='<div class="content"><h5>'.$lang['Statusinformationen'].'</h5>';
	...
	$status.='Browser: <img src="'.$BrowserIcon.'" hspace="8"><strong>'.MSD_BROWSER_AGENT.' Version '.MSD_BROWSER_VERSION.'</strong><br>';
	$status.='MySQL-Version: <strong>'.MSD_MYSQL_VERSION.'</strong><br>';
	$status.='PHP-Version: <strong>'.PHP_VERSION.'</strong> Speicher: <strong>'.$config["ram"].' MB</strong> '.(($config["zlib"]) ? '': '<span class="smallwarnung">&nbsp;&nbsp;'.$lang["phpbug"].'.&nbsp;&nbsp;</span>').$sm.'&nbsp;&nbsp;<a style="text-decoration:underline;" href="main.php?action=phpinfo">[Info]</a>&nbsp;&nbsp;';
	...
	$status.='<a href="main.php?action=extinfo&mysqlversion='.MSD_MYSQL_VERSION.'">['.$lang['Fehlerbericht'].']</a><br><br>';
	...
	//Datenbanken
	echo '<div class="content"><h5>'.$lang["info_databases"].'</h5><div style="padding-left:10px; font-weight:bold;">';
	...
	for($i=0;$i<count($databases["Name"]);$i++) {
		...
		if (!mysql_select_db($databases["Name"][$i],$config["dbconnection"])) {
			...
			$cl=($i==$databases["db_selected_index"]) ? "dbrowsel" : "dbrow";
			echo '<tr class="'.$cl.'"><td><a class="ul" href="main.php?action=db&dbid='.$i.'#dbid">';
	...
	echo '<div class="content"><h5>'.$lang['mysqlsys'].'</h5>';
	echo '';
	...
	echo '<div class="content"><h5>'.$lang['mysqlvars'].'</h5><div align="center"><table border="1" width="80%">';
	echo '<tr><td class="hd" align="center"><div align="left" class="green"><strong>'.$Titelausgabe[$var].'</strong></div>';
	...
	switch($var) {
		case "variables":
			...
			if($numrows==0) {echo $lang["info_novars"];} else {
				echo '<table><tr><td>Name</td><td>'.$lang['Inhalt'].'</td></tr>';
				for ($i = 0; $i < $numrows; $i++) {
			    	$row = mysql_fetch_array($res);
		...
		case "prozesse":
			...
			if($numrows==0) {echo $lang["info_noprocesses"];} else {
				echo '<table><tr nowrap><td>ID</td><td>User</td><td>Host</td><td>DB</td><td>Command</td><td>Time</td><td>State</td><td>Info</td><td nowrap>RT: '.round($config["processlist_refresh"]/1000).' sec</td></tr>';
				for ($i = 0; $i < $numrows; $i++) {
			    	$row = mysql_fetch_array($res);
...
if (isset($_GET["dbid"]))
{
	...
	if($numrows==0) {
		...
		echo '<tr class="hellblau"><td colspan="2"><strong>'.$lang["info_sum"].'</strong></td><td align="right"><strong>'.number_format($s1,0,",",".").'</strong></td><td align="right"><strong>'.number_format($s2,0,",",".").' Bytes</strong></td><td><strong>'.$last_update.'</strong></td>';
	...
	if($numrows>0) echo '<td><input class="Formbutton" type="submit" name="empty'.$dbid.'" value="'.$lang["clear_database"].'" onclick="if (!confirm(\''.$edb.'\')) return false;"></td>';
	echo '<td><input class="Formbutton" type="submit" name="kill'.$dbid.'" value="'.$lang["delete_database"].'" onclick="if (!confirm(\''.$kdb.'\')) return false;"></td>';
	if($numrows>0) {
		echo '<td><input class="Formbutton" type="submit" name="optimize'.$dbid.'" value="'.$lang["optimize_databases"].'"></td>';
		echo '<td><input class="Formbutton" type="submit" name="check'.$dbid.'" value="'.$lang["check_tables"].'"></td>';
	}
	echo '</tr></table>';
}
echo MSDFooter();;
