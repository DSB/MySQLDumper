...
sub send_mail {
	...
	From    => $cronmailfrom ,
	To      => $cronmailto ,
	Subject => "MySQLDumper Perl Crondump: ".$dbname ,
	Type    => 'text/html',
	Data    => $Body;
	if($DoAttach==1 && $mp==0) {
		attach $msg
	...
	if($DoAttach==1 && $mp>0 && $cronmail_dump>0) { 
		foreach $datei(@mparray) {
			...
			From    => $cronmailfrom ,
			To      => $cronmailto ,
			Subject => "MySQLDumper Perl Crondump: $dbname - Attach ".$str[1]." of ".@mparray ,
			Type    => 'text/html',
			Data    => $BodyAttachOnly;
			attach $msg
			Type     => "application/gzip" ,
