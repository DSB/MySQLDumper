...
#anti-vulnerable
foreach($_REQUEST as $g=>$v) { $_REQUEST[$g]=my_strip_tags($v);}
...
if($search==0) //$context==0)
{
	...
	if ($context==3)
	{
		...
		if(isset($_POST['dbwantaction'])) {
			if(isset($_POST['db_createnew'])) {
				...
				if(!empty($newname)) {
					...
					if(isset($_POST['db_default_charset']))
					{
		...
		echo '<tr><td colspan="2"><input type="submit" name="db_createnew" value="'.$lang['create'].'" class="SQLbutton"></td></tr>';
		echo '</table>';

		echo '<br><table class="border">';
