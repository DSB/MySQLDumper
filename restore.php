...
if ($restore['filehandle'])
{
	...
	else
	{
		...
		$dauer=0;
		$restore['EOB']=false;
		// Disable Keys of actual table to spped up restoring
		if ($restore['actual_table'] > '') @mysql_query('/*!40000 ALTER TABLE `' . $restore['actual_table'] . '` DISABLE KEYS */;',$config['dbconnection']);
	...
	$aus[]=sprintf($lang['restore_db'],$databases['db_actual'],$config['dbhost']) . '<br>' . $lang['file'] . ': <b>' . $restore['filename'] . '</b><br>' . $lang['charset'] . ': <strong>' . $restore['dump_encoding'] . '</strong><br>';
	...
	if ($restore['anzahl_tabellen'] > 0) $aus[]=sprintf($lang['restore_tables_completed'],$tabellen_fertig,$to_do);
	else $aus[]=sprintf($lang['restore_tables_completed0'],$tabellen_fertig);
	
	$done=number_format($restore['eintraege_ready'],0,',','.');
	$to_do=number_format($restore['anzahl_eintraege'],0,',','.');
	if ($restore['anzahl_eintraege'] > 0) $aus[]=sprintf($lang['restore_run1'],$done,$to_do);
	else $aus[]=sprintf($lang['restore_run0'],$done);
	$aus[]=sprintf($lang['restore_run2'],$restore['actual_table']) . $lang['progress_over_all'] . '<br>';
	...
	if ($restore['anzahl_eintraege'] > 0 && $restore['tables_to_restore'] === false)
	{
		...
		$aus[]=' <b>' . $lang['unknown_number_of_records'] . '</b><br><br>';
	...
	//Status-Text
	$aus[]='<p class="small">' . zeit_format(time() - $restore['xtime']) . ', ' . $restore['aufruf'] . ' pages' . ( ( $restore['part'] > 0 ) ? ', file ' . $restore['part'] : '' ) . ( ( $restore['errors'] > 0 ) ? ', <span class="error">' . $restore['errors'] . ' errors</span>' : '' ) . ( ( $restore['notices'] > 0 ) ? ', <span class="notice">' . $restore['notices'] . ' notices</span>' : '' ) . '</p>';
	...
	if ($restore['EOB'])
	{
		...
		if ($restore['errors'] > 0) $aus[]=$lang['errors'] . ': ' . $restore['errors'] . '  <a href="log.php?r=3">&raquo; ' . $lang['view'] . '</a><br>';
		if ($restore['notices'] > 0) $aus[]=$lang['notices'] . ': ' . $restore['notices'] . '  <a href="log.php?r=3">&raquo; ' . $lang['view'] . '</a><br>';
		$aus[]="<br>&nbsp;&nbsp;&nbsp;<input class=\"Formbutton\" type=\"button\" value=\"" . $lang['back_to_minisql'] . "\" onclick=\"self.location.href='sql.php'\">";
		$aus[]="&nbsp;&nbsp;&nbsp;<input class=\"Formbutton\" type=\"button\" value=\"" . $lang['back_to_overview'] . "\" onclick=\"self.location.href='main.php?action=db&dbid=" . $databases['db_selected_index'] . "#dbid'\">";
		$RestoreFertig=1;
	}
	else
	{
		if ($restore['fileEOF'])
		{
			...
			if (!file_exists($config['paths']['backup'] . $nextfile))
			{
				...
				$aus[]=( $restore['part'] > 0 ) ? ', ' . $lang['file'] . ' ' . $restore['part'] : '';
	...
	$aus[]=$config['paths']['backup'] . $restore['filename'] . ' :  ' . $lang['file_open_error'];
...
$page_parameter='<form action="restore.php" method="POST" name="restore"></form>';
if ($RestoreFertig == 1)
{
