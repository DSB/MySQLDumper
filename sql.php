...
if(!$download) include_once("inc/header.php");
...
$aus.='<a href="main.php?action=db&dbid='.$dbid.'#dbid"><img src="images/arrowleft.gif" width="16" height="16" alt="'.$lang['sql_backdboverview'].'" border="0"></a>&nbsp;&nbsp;';
...
if($context<3) {
	$aus.='`<a href="sql.php?db='.$db.'&dbid='.$dbid.'" title="'.$lang['sql_tableview'].'"><strong>'.$db.'</strong></a>`  '.(($tablename!="") ? '<strong>'.$lang['table'].'</strong> `<a href="sql.php?db='.$db.'&dbid='.$dbid.'&tablename='.$tablename.'&context=2"><strong>'.$tablename.'</a>`' : '').'</strong>'; 
...
if($context==0) {
	...
	if(isset($_GET["readfile"]) && $_GET["readfile"]==1) {
		$aus.='<form action="'.$params.'" method="post" enctype="multipart/form-data"><table cellpadding="0" cellspacing="0" border="1" rules="rows" width="100%"><tr class="sqlNew"><td>';
		$aus.='SQL-File öffnen (auch gz möglich):</td><td><input type="file" name="upfile"></td>';
		$aus.='<td><input type="submit" name="submit_openfile" value=" öffnen "></td><td>(max 2 MB)</td>';
		$aus.='<td><INPUT TYPE="HIDDEN" NAME="MAX_FILE_SIZE" VALUE="2500000">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table></form>';
	}
	if(isset($_POST["submit_openfile"])) {
		//open file
		if (!isset($_FILES["upfile"]["name"]) || empty($_FILES["upfile"]["name"])) $aus.='<span class="swarnung">'.$lang["fm_uploadfilerequest"].'</span>';
		else {
			...
			$aus.='<span class="">geladenes File: <strong>'.$_FILES["upfile"]["name"].'</strong>&nbsp;&nbsp;&nbsp;'.byte_output(filesize($_FILES["upfile"]["tmp_name"])).'</span>';
	...
	$aus.='<div class="sqlbox" id="mysqlbox"><form action="sql.php" method="post">';
	//Titelzeile
	$aus.='<div class="wtitle">';
	$aus.='<a href="#" onclick="resizeSQL(0);"><img src="images/close.gif" width="16" height="16" alt="" border="0" vspace="0" hspace="0" align="bottom"></a>&nbsp;&nbsp;';
	$aus.='<a href="#" onclick="resizeSQL(1);"><img src="images/arrowup.gif" width="16" height="16" alt="show less" border="0" vspace="0" hspace="0" align="bottom"></a>';
	$aus.='&nbsp;<a href="#" onclick="resizeSQL(2);"><img src="images/arrowdown.gif" width="16" height="16" alt="show more" border="0" vspace="0" hspace="0" align="bottom"></a>&nbsp;&nbsp;&nbsp;';
	...
	if(!isset($_GET["readfile"])) $aus.= '&nbsp;&nbsp;<a href="'.$params.'&readfile=1"  title="read file"><img src="images/openfile.gif" width="16" height="16" alt="read file" border="0"></a>&nbsp;';
	$aus.='<a href="'.$mysql_help_ref.'" target="_blanc" title="Mysql-Hilfe"><img src="images/help16.gif" width="16" height="16" alt="" border="0"></a>';
	$aus.='</div>';
	
	//Eingabebox
	$aus.='<div class="sbox2" id="sbox2"><textarea class="sqleingabe" style="height:'.$config["interface_sqlboxsize"].'px;" name="tb_sql"  id="tb_sql">'.((isset($sql_loaded)) ? $sql_loaded : $sql["sql_statement"].$sql["order_statement"]).'</textarea>';
	$aus.='<br><span class="verysmall"">'.$lang['sql_warning'].'</span></div>';
...
if(isset($_GET["mode"]) && $context==0) {
	...
	if($_GET["mode"]=="kill") {
		...
		if($showtables==0) {
			...
			$aus.='<p class="green">'.$lang['sql_recorddeleted'].'</p>';
		} else {
			$sqlk= "Drop Table `$rk`";
			$res=MSD_query($sqlk) or die(SQLError($sqlk,mysql_error()));
			$aus.='<p class="green">'.sprintf($lang['sql_recorddeleted'],$rk).'</p>';
		}
	}
	if($_GET["mode"]=="empty") {
		...
		if($showtables==0) {
			...
			$sqlk= "TRUNCATE `$rk`";
			$res=MSD_query($sqlk) or die(SQLError($sqlk,mysql_error()));
			$aus.='<p class="green">'.sprintf($lang['sql_tableemptied'],$rk).'</p>';
		}
	}
	if($_GET["mode"]=="emptyk") {
		...
		if($showtables==0) {
			...
			$res=MSD_query($sqlk) or die(SQLError($sqlk,mysql_error()));
			$aus.='<p class="green">'.sprintf($lang['sql_tableemptiedkeys'],$rk).'</p>';
		}
	}
	if($_GET["mode"]=="edit") {
		$aus.='<div class="sqleditbox"><span style="font-size:12px;"><strong>'.$lang['sql_recordedit'].' '.$rk.'</strong></span>';
		...
		for($x=0; $x<count($row); $x++) { 
			...
			$feldnamen.=$str->name.'|';
			$aus.='<tr><td class="sqletbl">'.$str->name.'</td><td>';
			if($str->type=='blob')
				$aus.='<textarea cols="60" rows="4" name="'.$str->name.'">'.$row[$x].'</textarea>';
			else
				$aus.='<input type="text" size="60" name="'.$str->name.'" value="'.$row[$x].'">';
			$aus.='</td>';	
			$aus.= '<td class="sqletbl">&nbsp;</td></tr>'; //'.$str->type.'
		}
	...
	if($_GET["mode"]=="new") {
		$aus.='<div class="sqlnewbox"><span style="font-size:12px;"><strong>'.$lang['sql_recordnew'].'</strong></span>';
...
if($context==0){
	//Data-View
	echo $aus.'<h4>'.(($showtables==1)?$lang['sql_tableview']:$lang['sql_dataview']).'</h4>';
	if($showtables==0){
		...
		echo '<a class="ul" " href="'.$p.'&mode=new">'.$lang['sql_recordnew'].'</a>';
	} else {
		$p='sql.php?db='.$db.'&dbid='.$dbid.'&context=2';
		echo '<a class="ul" href="'.$p.'">'.$lang['sql_tablenew'].'</a>';
	}
	//Statuszeile
	echo '<p align="left">';
			...
			for ($i=0;$i<$numrows;$i++) {
				...
				for($x=0; $x<count($row); $x++) { 
					if($x==0) {
						...
						if($showtables==0) {
							$d.='<a href="'.$p.'&mode=kill" onclick="if(!confirm(\''.$lang['askdeleterecord'].'\')) return false;"><img src="images/delete.gif" alt="delete" width="11" height="13" border="0"></a>';
						} else {
							if($tabellenansicht==1) {
								$d.='<a href="sql.php?db='.$db.'&dbid='.$dbid.'&tablename='.$row[0].'&context=2"><img src="images/edit.gif" width="12" height="13" alt="'.$lang['edit'].'" title="'.$lang['edit'].'" border="0"></a>&nbsp;'.$nl.$nl;
								$d.='<a href="'.$p.'&mode=empty" onclick="if(!confirm(\''.sprintf($lang['asktableempty'],$row[0]).'\')) return false;"><img src="images/empty.gif" alt="'.$lang['empty'].'" title="'.$lang['empty'].'" width="11" height="13" border="0"></a>&nbsp;'.$nl.$nl;
								$d.='<a href="'.$p.'&mode=emptyk" onclick="if(!confirm(\''.sprintf($lang['asktableemptykeys'],$row[0]).'\')) return false;"><img src="images/emptykeys.gif" alt="'.$lang['emptykeys'].'" title="'.$lang['emptykeys'].'" width="11" height="13" border="0"></a>&nbsp;'.$nl.$nl;
								$d.='<a href="'.$p.'&mode=kill" onclick="if(!confirm(\''.sprintf($lang['askdeletetable'],$row[0]).'\')) return false;"><img src="images/delete.gif" alt="'.$lang['delete'].'" title="'.$lang['delete'].'" width="11" height="13" border="0"></a>&nbsp;'.$nl.$nl;
							}
						}
						$d.='</td>';
					}
					...
					$d.=($tabellenansicht==1 && $x==0) ? "<a class=\"datacell\" href=\"sql.php?db=$db&tablename=$row[0]&dbid=$dbid\">$data</a>" : $data;
					$d.='&nbsp;</td>';
				}
				if($i==0) echo '<tr>'.$t.'</tr>';	
				echo '<tr>'.$d.'</tr>';	
	...
	echo '<form name="sqlform" action="sql.php" method="get">
	...
	if(isset($_GET["new"])) {
		echo '<tr><td>'.($i+1).'</td><td>';
		echo '<input type="text" name="sqlname'.$i.'" id="sqlname'.$i.'" value="SQL '.($i+1).'"><br><div class="small" align="center">'.$lang["sql_library"].'<br>';
		echo '<select id="sqllib" onclick="InsertLib('.$i.');" class="small">';
	...
	//Tabellen
	echo $aus.'<h4>'.$lang['sql_tablesofdb'].' `'.$databases["Name"][$dbid].'` '.$lang['sql_edit'].'</h4>';
	if(isset($_GET["kill"])) {
		if($_GET["anz"]==1)
	...
	echo '<table border="1" rules="rows" cellpadding="0" cellspacing="0"><tr class="sqlNew"><td>'.$lang['new'].' '.$lang['sql_createtable'].': </td><td colspan="2"><input type="text" name="newtablename" size="30" maxlength="150"></td><td><input type="submit" name="newtablesubmit" value="'.$lang['sql_createtable'].'"></td></tr>';
	echo '<tr class="sqlNew"><td>'.$lang['sql_copytable'].': </td><td><input type="text" name="tablecopyname" size="20" maxlength="150"></td><td><select name="copyatt"><option value="0">'.$lang['sql_structureonly'].'</option>'.((MSD_NEW_VERSION) ? '<option value="1">'.$lang['sql_structuredata'].'</option>' : '').'</select></td><td><input type="submit" name="tablecopysubmit" value="'.$lang['sql_copytable'].'" '.(($table_edit_name=="") ? "disabled=\"disabled\"":"").'></td></tr></table>';
	...
	if($table_edit_name!="") {
		...
		echo '<tr class="sqlNew"><input type="hidden" name="table_edit_name" value="'.$table_edit_name.'">';
		...
		if(isset($_GET["newfield"]) || isset($_GET["editfield"]) || $field_fehler>0 || isset($_POST["newfield_posted"])) {
			...
			echo '<table border="1" rules="rows" cellpadding="0" cellspacing="2"><tr class="sqlNew"><td colspan="6" align="center" style="font-size:10pt;font-weight:bold;">'.((isset($_GET["editfield"])) ? $lang['sql_editfield']." `".$d_name."`" : $lang['sql_newfield']).'</td></tr><tr class="sqlNew">';
			...
			echo '<td class="small" align="center">Size&nbsp;<img src="images/help16.gif" alt="'.$lang["sql_enumsethelp"].'" title="'.$lang["sql_enumsethelp"].'" width="12" height="12" border="0"><br><input type="text" value="'.$d_size.'" name="f_size" size="3" maxlength="80" style="font-size:11px;"></td>';
			...
			echo '</tr><tr class="sqlNew"><td class="small" align="center">'.$lang['sql_indexes'].'<br>';
			echo '<img src="images/nokey.gif" width="12" height="13" border="0" alt="No Index"><input type="radio" class="radio" name="f_index_new" value="no" '.(($d_primary+$d_unique+$d_index+$d_fulltext==0) ? "checked" : "").'>&nbsp;';
			echo '<img src="images/primary.gif" width="12" height="13" border="0" alt="Primary Key"><input type="radio" class="radio" name="f_index_new" value="primary" '.(($d_primary==1) ? "checked" : "" ).'>&nbsp;';
			echo '<img src="images/unique.gif" width="13" height="12" border="0" alt="Unique Index"><input type="radio" class="radio" name="f_index_new" value="unique" '.(($d_unique==1) ? "checked" : "" ).'>&nbsp;';
			echo '<img src="images/key.gif" width="13" height="12" border="0" alt="Index"><input type="radio" class="radio" name="f_index_new" value="index" '.(($d_index==1) ? "checked" : "" ).'>&nbsp;';
			echo '<img src="images/fulltext.gif" width="13" height="12" border="0" alt="Fulltext Index"><input type="checkbox" class="checkbox" name="f_indexfull" value="1" '.(($d_fulltext==1) ? "checked" : "" ).'>&nbsp;</td>';
			
			echo '<td class="small" align="center" colspan="2" >'.$lang['collation'].'<br><select name="f_collate" style="font-size:11px;">'.CollationCombo($d_collate).'</select></td>';
			echo '<td class="small" align="center">'.$lang['sql_attributes'].'<br><select name="f_attribut" style="font-size:11px;">'.AttributeCombo($d_attribute).'</select></td>';
			echo '<td class="small" align="center">'.$lang['sql_atposition'].':<br><select name="f_position" style="font-size:11px;"><option value=""></option><option value="FIRST">'.$lang['sql_first'].'</option>';
			if($anz_fields>0) {
				for($i=0;$i<$anz_fields;$i++) {
					echo '<option value="AFTER `'.$fields_infos[$i]["name"].'`">'.$lang['sql_after'].' `'.$fields_infos[$i]["name"].'`</option>';
				}
			}	
			echo '</select></td><td align="center"><input type="submit" name="newfieldsubmit" value="'.((isset($_GET["editfield"])) ? $lang['sql_changefield'] : $lang['sql_insertfield']).'"></td></tr></table></form>';
		} else 
		...
		for($i=0;$i<$anz_fields;$i++) {
			if($i==0) echo '<tr><td class="hdl">&nbsp;</td><td class="hdl">Field</td><td class="hdl">Type</td><td class="hdl">Size</td><td class="hdl">NULL</td><td class="hdl">Key</td><td class="hdl">Attribute</td><td class="hdl">Default</td><td class="hdl">Extra</td><td class="hdl">Sortierung</td></tr>';
			echo '<tr><td>';
			echo '<a href="'.$p.'&editfield='.$i.'"><img src="images/edit.gif" alt="edit field" width="12" height="13" border="0"></a>&nbsp;&nbsp;';
			echo '<a href="'.$p.'&kill='.$fields_infos[$i]["name"].'&anz='.$anz_fields.'" onclick="if(!confirm(\''.$lang['askdeletefield'].'\')) return false;"><img src="images/delete.gif" alt="delete field" width="11" height="13" border="0"></a>&nbsp;&nbsp;';
			...
			//key
			if($fields_infos["_primarykey_"]==$fields_infos[$i]["name"]) echo '<img src="images/primary.gif" width="12" height="13" alt="Primary Key" border="0">';
			if(isset($fields_infos["_fulltextkey_"])) {
				for($ii=0;$ii<count($fields_infos["_fulltextkey_"]);$ii++) {
					if($fields_infos["_fulltextkey_"][$ii]["name"]==$fields_infos[$i]["name"]) {
						echo '<img src="images/fulltext.gif" width="13" height="12" alt="Fulltext Index" border="0">'; break;
					}
				}
			}
			if(isset($fields_infos["_uniquekey_"])) {
				for($ii=0;$ii<count($fields_infos["_uniquekey_"]);$ii++) {
			 		if($fields_infos["_uniquekey_"][$ii]["name"]==$fields_infos[$i]["name"]) {
						echo '<img src="images/unique.gif" width="13" height="12" alt="Unique Index" border="0">'; break;
					}
				}
			}
			if(isset($fields_infos["_key_"])) {
				 for($ii=0;$ii<count($fields_infos["_key_"]);$ii++) {
				 	...
				 	if($fields_infos["_key_"][$ii]["name"]==$fields_infos[$i]["name"]) {
						echo '<img src="images/key.gif" width="13" height="12" alt="Index" border="0">'; break;
					}
				 }
			}
		...
		if($num==0) {
			...
			for($i=0;$i<$num;$i++) {
				$row=mysql_fetch_array($res);
				//Images
				echo '<tr><td><img src="images/edit.gif" width="12" height="13" alt="" border="0">&nbsp;&nbsp;<img src="images/delete.gif" width="12" height="13" alt="" border="0"></td>';
	...
	echo '<tr class="sqlNew"><td colspan="3" align="center"><strong>'.$lang["create_database"].'</strong></td></tr>';
	echo '<tr class="sqlNew"><td colspan="3">Name:&nbsp;<input type="text" name="db_create" size="20">';
	echo '&nbsp;&nbsp;'.$lang['collation'].':&nbsp;<select name="db_collate" style="font-size:11px;">'.CollationCombo('',1).'</select>&nbsp;&nbsp;<input type="submit" name="db_createnew" value="anlegen"></td></tr>';
	echo '<tr><td class="hd">'.$lang["dbs"].'</td><td class="hd">'.$lang['sql_actions'].'</td></tr>';
	for($i=0;$i<count($databases["Name"]);$i++) {
		echo ($i==$databases["db_selected_index"]) ? '<tr class="dbrowsel">' : '<tr class="'.(($i % 2)  ? 'dbrow' : 'dbrow1').'">';
		echo '<td><a class="datacell" href="sql.php?db='.$databases["Name"][$i].'&dbid='.$i.'">'.$databases["Name"][$i].'</a></td>';
		echo '<td nowrap><input type="text" name="db_rename'.$i.'" size="20">';
		...
		echo '&nbsp;&nbsp;<input type="Button" value="'.$lang["sql_imexport"].'" onclick="location.href=\'sql.php?db='.$databases["Name"][$i].'&dbid='.$i.'&context=4\'"></td></tr>';
	}
	...
	if($import==1) {
		...
		if(isset($_POST["do_import"])) {
			...
			if($sql["import"]["table"]=="") {
				$aus.='<span class="swarnung">'.$lang["import_notable"].'</span>';
			} else {
				if($_POST["import_source"]==0) {
					...
					if (!isset($_FILES["upfile"]["name"]) || empty($_FILES["upfile"]["name"])) {
						$aus.='<span class="swarnung">'.$lang["fm_uploadfilerequest"].'</span>';
					} else {
						$fn=$_FILES["upfile"]["tmp_name"];
						
						$sql["import"]["csv"]=(substr($_FILES["upfile"]["name"],-3)==".gz") ? gzfile($fn) : file($fn);
						$sql["import"]["text"]=implode("",$sql["import"]["csv"]);
						$aus.='<span class="">'.$lang["sql_uploadedfile"].'<strong>'.$_FILES["upfile"]["name"].'</strong>&nbsp;&nbsp;&nbsp;'.byte_output(filesize($_FILES["upfile"]["tmp_name"])).'</span>';
				...
				if(is_array($sql["import"]["csv"])) 
					...
					$aus.='<br><span class="swarnung">'.$lang["csv_nodata"].'</span>';
				
			}
		}
		$impaus=$aus.'<h4>'.sprintf($lang['sql_import'],$databases["Name"][$dbid]).'</h4>';
		
		$impaus.='<form action="sql.php?db='.$db.'&dbid='.$dbid.'&context=4&import=1" method="post" enctype="multipart/form-data">'.$nl;
		$impaus.= '<table border="1"><tr class="sqlNew"><td nowrap><a class="ul" href="sql.php?db='.$db.'&dbid='.$dbid.'&context=4">'.$lang["export"].'</a><div align="center"><strong>'.$lang["import"].'</strong></div></td></tr><tr class="sqlNew"><td><table><tr><td valign="top">'.$nl;
		$impaus.= '<h5>'.$lang["importoptions"].'</h5></td><td valign="top"><h5>'.$lang["csvoptions"].'</h5></td></tr>'.$nl;
		
		$impaus.= '<tr><td valign="top">'.$nl;
		$impaus.= '<div><table cellpadding="0" cellspacing="0">'.$nl;
		...
		$impaus.= '<tr><td>'.$lang["csv_fieldseperate"].'</td><td><input type="text" name="f_import_csvtrenn" size="4" maxlength="12" value="'.$sql["import"]["trenn"].'"></td></tr>'.$nl;
		$impaus.= '<tr><td>'.$lang["csv_fieldsenclosed"].'</td><td><input type="text" name="f_import_csvenc" size="4" maxlength="12" value="'.htmlspecialchars($sql["import"]["enc"]).'"></td></tr>'.$nl;
		$impaus.= '<tr><td>'.$lang["csv_fieldsescape"].'</td><td><input type="text" name="f_import_csvesc" size="4" maxlength="12" value="'.$sql["import"]["esc"].'"></td></tr>'.$nl;
		$impaus.= '<tr><td>'.$lang["csv_eol"].'</td><td><input type="text" name="f_import_csvztrenn" size="4" maxlength="12" value="'.$sql["import"]["ztrenn"].'"></td></tr>'.$nl;
		$impaus.= '<tr><td>'.$lang["csv_null"].'</td><td><input type="text" name="f_import_csvnull" size="4" maxlength="12" value="'.$sql["import"]["null"].'"></td></tr>'.$nl;
  		$impaus.= '</table></div>'.$nl;
		...
		$impaus.= '</table></td></tr></table>'.$nl;
		

		
		$impaus.= '<br><br>'.$lang["import"].':<br><textarea name="import_text" wrap="OFF" style="width:760px;height:400px;font-size=11px;">'.$sql["import"]["text"];
		...
		$exaus.= '<table border="1"><tr class="sqlNew"><td nowrap><a class="ul" href="sql.php?db='.$db.'&dbid='.$dbid.'&context=4&import=1">'.$lang["import"].'</a><div align="center"><strong>'.$lang["export"].'</strong></div></td></tr><tr class="sqlNew"><td><table><tr><td valign="top"><h5>'.$lang["tables"].'</h5>'.$nl;
		$exaus.= '<span class="verysmall"><strong>'.$sql["export"]["tablecount"].'</strong> '.$lang["tables"].', <strong>'.$sql["export"]["recordcount"].'</strong> '.$lang["records"].'</span><br><select name="f_export_tables[]" size="12" multiple>'.$tblstr.'</select><br>'.$nl;
		$exaus.= '<a class="verysmall" href="#" onclick="SelectTableList(true);">'.$lang["all"].'</a>&nbsp;&nbsp;<a class="verysmall" href="#" onclick="SelectTableList(false);">'.$lang["none"].'</a>'.$nl;
		$exaus.= '</td><td valign="top">'.$nl;
		$exaus.= '<h5>'.$lang["exportoptions"].'</h5>'.$nl;
		...
		$exaus.= '<input type="radio" class="radio" name="f_export_format" id="radio_csv2" value="2" '.(($sql["export"]["format"]==2) ? "checked" : "").' onclick="check_csvdivs(0); return true">'.$lang["excel2003"].'<br><br>'.$nl;
		$exaus.= '<div id="csv0"><table cellpadding="0" cellspacing="0"><tr><td colspan="2">'.$nl;
		...
		$exaus.= '<tr><td>'.$lang["csv_fieldseperate"].'</td><td><input type="text" name="f_export_csvtrenn" size="4" maxlength="12" value="'.$sql["export"]["trenn"].'"></td></tr>'.$nl;
		$exaus.= '<tr><td>'.$lang["csv_fieldsenclosed"].'</td><td><input type="text" name="f_export_csvenc" size="4" maxlength="12" value="'.htmlspecialchars($sql["export"]["enc"]).'"></td></tr>'.$nl;
		$exaus.= '<tr><td>'.$lang["csv_fieldsescape"].'</td><td><input type="text" name="f_export_csvesc" size="4" maxlength="12" value="'.$sql["export"]["esc"].'"></td></tr>'.$nl;
		$exaus.= '<tr><td>'.$lang["csv_eol"].'</td><td><input type="text" name="f_export_csvztrenn" size="4" maxlength="12" value="'.$sql["export"]["ztrenn"].'"></td></tr>'.$nl;
		$exaus.= '<tr><td>'.$lang["csv_null"].'</td><td><input type="text" name="f_export_csvnull0" size="4" maxlength="12" value="'.$sql["export"]["null"].'"></td></tr>'.$nl;
  		$exaus.= '</table></div>'.$nl;
		$exaus.= '<div id="csv1"><table cellpadding="0" cellspacing="0"><tr><td colspan="2">';
		...
		$exaus.= '<tr><td>'.$lang["csv_null"].'</td><td><input type="text" name="f_export_csvnull1" size="4" maxlength="12" value="'.$sql["export"]["null"].'"></td></tr>'.$nl;
  		$exaus.= '</table></div>'.$nl;
		
		$exaus.= '</td><td>&nbsp;&nbsp;&nbsp;</td>'.$nl;
		$exaus.= '<td valign="top"><h5>'.$lang["export"].'</h5>'.$nl;
		...
		$exaus.= '<img src="images/blank.gif" width="60" height="130" border="0"><br><input class="Formbutton" type="submit" name="f_export_submit" value="exportieren" onclick="if(SelectedTableCount()==0) {alert(msg1);return false;}">'.$nl;
		$exaus.= '</td></tr></table></td></tr></table></form>'.$nl;
		...
		if(isset($_POST["f_export_submit"]) && isset($sql["export"]["tables"])) {
			if(!$download) echo '<br><br>'.$lang["export"].':<br><textarea wrap="OFF" style="width:760px;height:400px;font-size=11px;">'.$nl;
			Exportcsv();
			if(!$download) {
				echo '</textarea>'.$nl;
